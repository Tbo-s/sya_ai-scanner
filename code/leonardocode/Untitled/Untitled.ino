...
const int gateOpenSwitch  = A4;  // bovenste gate-stopper
const int gateCloseSwitch = A5;  // onderste gate-stopper
const int trayOutSwitch   = 4;
const int trayInSwitch    = 5;

// All stopper switches are normally closed to GND with INPUT_PULLUP:
// idle/not hit = LOW, hit/opened = HIGH.
const int STOPPER_SWITCH_TRIGGERED_LEVEL = HIGH;
...
void stopGate();
void stopTray();
bool isGateOpenStopPressed();
bool isGateCloseStopPressed();
bool hasGateStopConflict();
bool isTrayOutStopPressed();
bool isTrayInStopPressed();
bool hasTrayStopConflict();
...
void processCommand(Print &output, const char *cmd) {
  if (strcmp(cmd, "GATE_OPEN") == 0) {
    updateGatePositionFromSwitches();
    if (hasGateStopConflict()) {
      stopGate();
      gateState = GATE_IDLE;
      output.println("ERR:GATE_STOP_CONFLICT");
      return;
    }
    if (isGateOpenStopPressed()) {
      stopGate();
      gateState = GATE_IDLE;
    ...
    updateTrayPositionFromSwitches();
    if (hasTrayStopConflict()) {
      stopTray();
      trayState = TRAY_IDLE;
      output.println("ERR:TRAY_STOP_CONFLICT");
      return;
    }
    if (isTrayOutStopPressed()) {
      stopTray();
      trayState = TRAY_IDLE;
...
bool isGateOpenStopPressed() {
  return digitalRead(gateOpenSwitch) == STOPPER_SWITCH_TRIGGERED_LEVEL;
}

bool isGateCloseStopPressed() {
  return digitalRead(gateCloseSwitch) == STOPPER_SWITCH_TRIGGERED_LEVEL;
}

bool hasGateStopConflict() {
  return isGateOpenStopPressed() && isGateCloseStopPressed();
}

bool isTrayOutStopPressed() {
  return digitalRead(trayOutSwitch) == STOPPER_SWITCH_TRIGGERED_LEVEL;
}

bool isTrayInStopPressed() {
  return digitalRead(trayInSwitch) == STOPPER_SWITCH_TRIGGERED_LEVEL;
}

bool hasTrayStopConflict() {
  return isTrayOutStopPressed() && isTrayInStopPressed();
}
...
// =========================
void updateGatePositionFromSwitches() {
  bool openPressed  = isGateOpenStopPressed();
  bool closePressed = isGateCloseStopPressed();
...
void updateTrayPositionFromSwitches() {
  bool outPressed = isTrayOutStopPressed();
  bool inPressed  = isTrayInStopPressed();
